...
try:
    ...
    # creates the connection to the pymongo db
...
# ENDPOINTS are the API requests that are made to Apiary
...
def collect_samples():
    ...
    hours = 1
    key = '/home/blebiednik3/Brian_unencrypted_key.pem'
    global conn
    # connection
    ...
    # creates the session with Apiary
    ...
    for name in ENDPOINTS:
        for endpoint, parameters in ENDPOINTS[name].iteritems():
            ...
            try:
                ...
                data = resp.json()
                # Does the collection of the information that we requested
                # in ENDPOINTS
                for doc in data.get('results', []):
                    if name == 'dns_queries':
                        ...
                        # Instead of providing the number times the malware
                        # tried to access it provides the hashes of the malware
                        # that tried to access the domain, here we count the
                        # hashes
    ...
    """for ip in url_ip_coll.find({'IP': 'Null'}, no_cursor_timeout=True):
        ...
        try:
            ...
            url_ip_coll.update({'DNS': ip['DNS']},
                               {'$set': {'IP': 'Unresolved'}})
    This was removed because the time that it takes to do the lookups
    is too long for the scheduling so it is done separtely
    """
    for ip in url_ip_coll.find({'Attribute': 'PUBLIC'}):
        ip_dict[ip['IP']] += ip['seen']
        # Just to show that it is working I collect the top PUBLIC IPS

    for dns in url_ip_coll.find({'Alexa': {"$gt": alexa}}):
        dns_dict[dns['DNS']] += dns['seen']
        # Also just to show that it is working I collected all DNS requests
        # with an Alexa ranking greater than 10000
    print '\n Top {} DNS request:'.format(top_number)
    for k, v in dns_dict.most_common(top_number):
        print '%s: %i' % (k, v)
        # print most common DNS request and number seen
    print '\n Top {} IP request:'.format(top_number)
    for k, v in ip_dict.most_common(top_number):
        print '%s: %i' % (k, v)
        # print most common IP requests and number seen


def clean_db():
    """Remove old data. Currently removes everything over 14 days old."""
    global conn
    # Connection
    dns_ip_db = conn.dns_ip_db
    # Database
